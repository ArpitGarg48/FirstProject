      ...
      <header className="flex flex-col gap-4 bg-white px-6 py-4 shadow-sm sm:flex-row sm:items-center sm:justify-between">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
          ...
          <nav className="flex flex-wrap gap-2 text-sm text-slate-600">
            ...
            <button className="rounded-full px-4 py-2 hover:bg-slate-100" onClick={()=>{navigate("/panel/attendence")}}>
              Attendance
            </button>
            <button className="rounded-full px-4 py-2 hover:bg-slate-100" onClick={()=>{navigate("/panel/payroll")}}>
              Payroll
            </button>
          </nav>
        </div>
        ...
        <aside className="rounded-3xl bg-white p-6 shadow-sm">
          ...
          <ul className="space-y-2 text-sm text-slate-600">
            {[
              ...
              {label:"Attendance",path:"/panel/attendence"},
              {label:"Leave Requests",path:"/panel/leave"},
              {label:"Payroll",path:"/panel/payroll"},
              {label:"Performance",path:"/panel/performance"},
              {label:"Reports",path:"/panel/report"},
              {label:"Settings",path:"/panel/setting"},
            ].map((item) => (
              <li
